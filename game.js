...
function updateMatchUI(){
  ...
  $("shotPanel").style.display=isBat&&userOn?"block":"none";$("bowlPanel").style.display=isBat?"none":"block";document.querySelectorAll("[data-shot],[data-bowl]").forEach(b=>b.disabled=!!anim.active);
...
function battingShot(type){
  ...
  if(wicket){game.wickets++;game.playerOut=true;game.userStriker=false;setPhaseText(`${SHOT_NAMES[type]} • outside edge! WICKET — ${career.player.name} is out for ${game.playerRuns}.`);pushBallEvent(0,true,Math.floor((game.balls-1)/6),((game.balls-1)%6)+1,SHOT_NAMES[type])}
