...
$("topStats").textContent=`SEASON ${p.season.number} • ${p.team} • ₹ ${fmt(p.money)}`;
  ...
  const wd=$("worldDashboard"); if(wd) wd.innerHTML=`<div class="worldGrid"><div><small>RIVAL</small><b>${w.rival.name}</b><span>${w.rival.runs} runs • ${w.rival.matches} matches</span></div><div><small>LEAGUE</small><b>${w.league.points} pts</b><span>${w.league.wins}W ${w.league.losses}L • NRR ${Number(w.league.nrr||0).toFixed(2)}</span></div><div><small>RIVAL FORM</small><b>${Math.round(w.rival.form)}</b><span>${w.rival.wins} wins</span></div></div>`;
...
function bowling(type){
  ...
  if(wicket){game.aiWickets++;setPhaseText(`${d.desc} • OUT! ${b.name} is beaten by your ${type}.`);pushBallEvent(0,true,Math.floor((game.aiBalls-1)/6),((game.aiBalls-1)%6)+1,type.toUpperCase());advanceAIWicket()}
  else{game.aiScore+=runs;if([1,3,5].includes(runs))game.aiOnStrike=!game.aiOnStrike;$("aiDecision").textContent=`AI played ${aiShot.toUpperCase()} • required RR ${required.toFixed(2)} • ${b.name} ${b.form} form`;setPhaseText(`${b.name}: ${runs===0?"dot ball":runs===4?"FOUR":runs===6?"SIX":`${runs} run${runs>1?"s":""}`}.`);pushBallEvent(runs,false,Math.floor((game.aiBalls-1)/6),((game.aiBalls-1)%6)+1,type.toUpperCase())}
  if(game.aiScore>=game.target||game.aiBalls>=120||game.aiWickets>=10){finishMatch();return}
  if(game.aiBalls%6===0){game.aiOnStrike=!game.aiOnStrike;game.overNumber++;setPhaseText(`OVER ${game.overNumber} COMPLETE • New over, strike rotated. ${Math.max(0,game.target-game.aiScore)} needed.`)}updateMatchUI();if(autoPlayMode&&!game.finished)scheduleAutoStep(220);
}
...
function finishMatch(){
  if(!game||game.finished)return;
  clearAutoTimer();autoPlayMode=false;paused=false;matchLock=false;fallbackBall=null;const po=$("pauseOverlay");if(po)po.classList.remove("show");game.finished=true;game.phase="finished";const tie=game.aiScore===game.target-1;const win=game.aiScore<game.target-1;career.applyMatch({win,runs:game.playerRuns,balls:game.playerBalls,wickets:game.aiWickets,catches:game.catches,sixes:game.sixes,fours:game.fours,opponent:game.opponent.name,notOut:!game.playerOut});renderAll();$("resultIcon").textContent=win?"🏆":"💪";$("resultTitle").textContent=win?(tie?"MATCH WON!":"MATCH WON!"):"MATCH LOST";$("resultText").textContent=win?`${career.player.name}'s XI defended ${game.target-1} against ${game.opponent.name}.`:`${game.opponent.name} chased ${game.target} with ${Math.max(0,120-game.aiBalls)} balls remaining.`;$("resultStats").innerHTML=[["Your runs",game.playerRuns],["Team score",game.target-1],["AI score",game.aiScore],["AI wickets",game.aiWickets],["Balls",game.playerBalls],["Fours / Sixes",`${game.fours}/${game.sixes}`],["Form",Math.round(career.player.form)],["Fitness",Math.round(career.player.fitness.condition)]].map(x=>`<div class="resultStat"><small>${x[0]}</small><b>${x[1]}</b></div>`).join("");$("matchResult").classList.add("show");if(camera)cinematicCamera(0);
}
